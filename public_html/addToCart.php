<?php
    ...
    if(isset($_POST['submit'])) {
        ...
        $sql = "INSERT INTO ordered_item (item_id, user_id,  size, color, quantity) VALUES (". $id .", ". $_SESSION['user_id'] .", ". $_POST['size'] .", '". $_POST['color'] ."', '". $_POST['quantity'] ."');";
...
<html>
   <head>
    ...
    <title>Add to Cart - The Fashion Shop</title>
  ...
  <body>
    ...
    <header class="masthead">
      <div class="container">
        <div class="intro-text">
          ...
          <div class="intro-heading text-uppercase" style="font-family:'Lithos Pro Regular';">Products</div>
		   <div class="intro-lead-in">Choose your fashion style</div>
	...
	<br/>
       ...
       <div class="note4">Product Specifications</div>
                ...
                if($conn = mysqli_connect('localhost', 'root', '', 'the_fashion_shop')) {
                    ...
                    if($result = mysqli_query($conn, $sql)) {
                        if($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo '<img src="img/'.$row['image'].'" style="width:100%" height="90%">';
                            echo '</div>';
                            echo '<div class="col-sm-7">';
                            echo '<h3>'. $row['name'] .'</h3>';
                            echo '<h5>RM'. $row['price'] .'</h5>';
                            echo '<p> Description: '. $row['description'] .'</p>';
                            echo '<form name="addToCartForm" action="addToCart.php?id='. $id .'" onsubmit="return validateForm()" method="POST">';                 
                                 echo '<hr><center><b>-Select Variation-</b></center>';                              
 
                                echo '<p>Size:</p>';
                                echo '<select name="size" id="size" class="form-control">
                                                <option value="0">S / M / L </option>
                                                <option value="1">S (96-101cm/37.8-39.8")</option>
                                                <option value="2">M (101-106cm/39.8-41.7")</option>
                                                <option value="3">L (106-111cm/41.7-43.7")</option>
                                        </select>';

                                 echo '<br/>';
        
                                echo '<p>Color:</p>';
                                echo '<input type="text" class="input-white" id="color" name="color" placeholder="Enter color variation" />';
            
                                echo '<p>Quantity: &emsp;
                                      <button type="button" class="btn qtyminus" title="Minus" onclick="decrementValue()"><i class="fas fa-minus"></i></button>&nbsp;';
                                echo '<input type="num" name="quantity" value="1" class="qty" id="number" maxlength="2" max="100" size="1"/>';
                                echo '&nbsp;<button type="button" class="btn qtyplus" title="Add" onclick="incrementValue()"><i class="fas fa-plus"></i></button></p>';
                                echo '<hr>';
                                
                                echo '<button type="submit" name="submit" class="btnAddCart btn" title="Add to Cart"><i class="fas fa-cart-plus"></i>&nbsp;<b>Add to Cart</b></button>  ';
  ...
  <script>
         ...
         function validateForm() {
            ...
            var b = document.forms["addToCartForm"]["color"].value;
            
                if (Size == "0" && b == "") {
                  alert("Please choose your size & color");
                  return false;
                }else if (Size == "0") {
                  alert("Please choose your size");
                  return false;
                }else if (b == "") {
                  alert("Please fill in the color");
                  return false;
                }
        }
